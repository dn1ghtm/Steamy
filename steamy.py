...
class SteamyLauncher:
    ...
    def display_menu(self, games):
        ...
        # Dynamic quick actions based on configuration state
        if not self.library_paths:
            quick_actions = ["Set Library", "Auto-detect", "Quit"]
        elif not self.current_user:
            quick_actions = ["Select User", "Settings", "Quit"]
        else:
...
{' ' * 35}                                                \\/__/ {Style.RESET_ALL}"""
        ...
        while True:
            ...
            # Games list with modern styling - removed right vertical bars
            print(f"{Fore.BLUE}┌{'─' * 118}")
            ...
            if not self.library_paths:
                ...
                print(f"│{' ' * 25}{Fore.WHITE}To get started, go to Settings and configure your Steam libraries.{Style.RESET_ALL}{' ' * 25}")
                print(f"│{' ' * 30}{Fore.WHITE}You can auto-detect libraries or add them manually.{Style.RESET_ALL}{' ' * 35}")
                print(f"│{' ' * 118}")
                ...
                print(f"│{' ' * 25}{Fore.WHITE}Go to Settings and select a Steam user to start playing.{Style.RESET_ALL}{' ' * 30}")
                print(f"│{' ' * 30}{Fore.WHITE}This will allow you to launch games directly.{Style.RESET_ALL}{' ' * 35}")
                print(f"│{' ' * 118}")
            elif games:
                ...
                for i in range(0, len(games), 3):
                    ...
                    if game2 and game3:
                        print(f"{Fore.BLUE}│{Style.RESET_ALL} {Fore.WHITE}{i+1:2d}.{Style.RESET_ALL} {game1['name'][:35]:<35} {Fore.WHITE}{i+2:2d}.{Style.RESET_ALL} {game2['name'][:35]:<35} {Fore.WHITE}{i+3:2d}.{Style.RESET_ALL} {game3['name'][:35]:<35}")
                    elif game2:
                        print(f"{Fore.BLUE}│{Style.RESET_ALL} {Fore.WHITE}{i+1:2d}.{Style.RESET_ALL} {game1['name'][:35]:<35} {Fore.WHITE}{i+2:2d}.{Style.RESET_ALL} {game2['name'][:35]:<35} {' ' * 40}")
                    else:
                        print(f"{Fore.BLUE}│{Style.RESET_ALL} {Fore.WHITE}{i+1:2d}.{Style.RESET_ALL} {game1['name'][:35]:<35} {' ' * 75}")
                ...
                print(f"│{' ' * 25}{Fore.WHITE}Try refreshing or check your library paths in Settings.{Style.RESET_ALL}{' ' * 30}")
                print(f"│{' ' * 118}")
            
            # Modern quick actions bar with arrow navigation - removed right vertical bars
            print(f"{Fore.BLUE}├{'─' * 118}")
            ...
            # Show actions with selection indicator and library count on the right
            actions_text = ""
            for i, action in enumerate(quick_actions):
                if i == selected_action:
                    actions_text += f"{Fore.WHITE}>{action}{Style.RESET_ALL}"
                else:
                    actions_text += action
                if i < len(quick_actions) - 1:
                    actions_text += " | "
            
            library_info = f"{Fore.BLUE}Libraries: {Style.RESET_ALL}{len(self.library_paths)}"
            padding = 110 - len(actions_text) - len(library_info.replace(f"{Fore.BLUE}", "").replace(f"{Style.RESET_ALL}", ""))
            print(f"{Fore.BLUE}│{Style.RESET_ALL} {actions_text}{' ' * padding}{library_info}")
            
            print(f"└{'─' * 118}{Style.RESET_ALL}")
            
            # Get user input with modern prompt
            print(f"\n{Fore.BLUE}Use arrow keys for quick actions, number for games: {Style.RESET_ALL}", end='', flush=True)
            
            # Handle input with dynamic actions
            key = self.get_key()
            if key in ['LEFT', 'RIGHT']:
                ...
                if not self.library_paths:
                    if selected_action == 0:  # Set Library
                        self.manage_library_paths()
                        games = self.get_installed_games()
                        # Update menu to show Select User since libraries are configured but no user is selected
                        quick_actions = ["Select User", "Settings", "Quit"]
                        selected_action = 0
                    elif selected_action == 1:  # Auto-detect
                        auto_paths = self.find_steam_libraries()
                        if auto_paths:
                            for path in auto_paths:
                                if path not in self.library_paths:
                    ...
                    if selected_action == 0:  # Select User
                        self.select_steam_user()
                        # Update menu to show Select User since libraries are configured but no user is selected
                        quick_actions = ["Select User", "Settings", "Quit"]
                        selected_action = 0
                    elif selected_action == 1:  # Settings
                        self.settings_menu()
                        games = self.get_installed_games()
                    elif selected_action == 2:  # Quit
                        return
